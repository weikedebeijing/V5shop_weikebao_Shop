...
//difine the AjaxStatus that used to check the ajax is complete,default value is 0 --add by zyh
this.jqueryAjaxStatus = -1;
...
jQuery.extend({
	...
	ajax: function( s ) {
		...
		//make sure the request does'nt read from cache in ie --add by zyh
		var correctUrl = "";
		if(s.url.indexOf("?") != -1)
			correctUrl = s.url+"&roid="+Math.random();
		else
			correctUrl = s.url+"?roid="+Math.random();
			
		if( s.username )
			xhr.open(type, correctUrl, s.async, s.username, s.password);
		else
			xhr.open(type, correctUrl, s.async);
		...
		var onreadystatechange = function(isTimeout){
			//set the global jqueryAjaxStatus variable --add by zyh
			this.jqueryAjaxStatus = xhr.readyState;
		...
		function success(){
			...
			//when data contains error message,process it  --add by zyh
			if(!processErrorMessage(data)) return;
		...
		//process the error message  --add by zyh
		function processErrorMessage(bakValue)
		{
		   if(bakValue!=null && typeof bakValue == "string")
		   {
			  if(bakValue.indexOf('error_')==0)
			  {
				 if(bakValue.length>6){bakValue=bakValue.substr(6);}
				 else{bakValue='程序发生了未知错误，请再次尝试！';}
				 
				 //if return message contains script ,then invoke the script self
				 if(bakValue.match(/<script [^>]*>|<script>/ig))
				 	$("body").append(bakValue);
				 else
				 	alert(bakValue);
				 return false;
			  }
