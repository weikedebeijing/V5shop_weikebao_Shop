...
function changecat(obj, level){
	...
	if(level == 2){
		$('areafid_' + (level+1)).style.display = "none";
	}
...
function VaildData()
{
    ...
    //{
		...
		//判断用户名长度
		if(tmp.value.length < 2 || tmp.value.length > 16)
        {
...
//确定按钮提交后返回事件
function VaildUID_Return(txt1, txt2, strReturn)
{
    var uid = getobj("txtUID");
	//var strReturn = getobj("tmpreg").innerHTML;
...
function btnVaildUID_onclick() {
    ...
    //{
		...
		//判断用户名长度
		if(txtUid.value.length < 2 || txtUid.value.length > 16)
		{
			alert("用户名必须在2~16字符之间！");
			txtUid.select();
			return false;
		...
		getobj('vaildTxtUID').value = txtUid.value;
		...
		/*var myajax = new Ajax.Updater(    
			...
			{
				method: 'post',    
				parameters : Form.serialize($('VaildForm')),
				onComplete : VaildUID_Return  
			}
		);*/
        $('btnVaildUID').disabled = true;

var myHTMLRequest = new Request.HTML({url:Url,onComplete:VaildUID_Return}).post({_OP:"OP_VaildUID", txtUID:txtUid.value});


	//$('tmpreg').set('load', myHTMLRequest)
	//$('tmpreg').load(Url);

		
		//AjaxSubmit(Url, PostData, VaildUID_Return);  {_OP:"OP_VaildUID", txtUID:encodeURI(txtUid.value)}
